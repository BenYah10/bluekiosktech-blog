// /api/oauth.js — GitHub OAuth pour Decap CMS (super-compat, ESM)
// - Compatible ESM (Vercel + "type":"module")
  ...
  (function () {
    ...
    try {
      if (window.opener && !window.opener.closed) {
        // 1) Stockage local fallback (2 clés)
        try {
...
export default async function handler(req, res) {
  try {
    ...
    return res
      ...
      .end(
        htmlError(
          (e && (e.stack || e.message)) || "Unhandled serverless error"
        )
      );
  }
}
